...
- [[Linear Project Structure]] — **initiative = solution, project = delivery segment, milestones group deliveries.** The shape msilva must migrate the proxy backlog *into* — and **Claude writes that structure from the design docs**, which is the area's routine practice. Plus teams-as-isolation, Luís's templates (he's rewriting them), and the Slack integration. Conventions explicitly provisional and msilva is invited to change them. Free cap ~250 issues, **already hit once**; Business trial expires 2026-09-09. **2026-08-19**: only the *active* project needs real structure now, the rest of the restored Jira backlog can just be deleted; msilva has full autonomy on Linear org decisions; `Release` added as an unresolved concept.
- [[Claude Agent SDK]] — running Claude Code via API/CLI, no IDE. Introduced by Luís, 2026-08-19, as a topic to track; not yet tied to a specific [[Agent Flow]] agent.
...
- [[Meeting prep - A5 Watcher discovery with Carol - 2026-08-19]] — agenda for the second of two deliberately-separate discovery conversations on Watcher/A5 scope (Gabrielle's side already held earlier the same day). Cold opening question first, background and Carol-specific threads (her own-pain criterion, her intranet tool as possible A10, skills-repo overlap with A6) held back until after she answers.
