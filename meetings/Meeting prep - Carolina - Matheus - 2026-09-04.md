...
| Onde o A10 publica hoje? | — | Status update nativo da **iniciativa** no Linear (`publish_initiative_update`/`projectUpdateCreate`) — um post agregado, não comentário em issue individual. Foi revertido de comentário-por-issue em 2026-09-01. **Mudou de novo hoje (2026-09-04, 13h49, `7fcb354`)**: até então publicava por projeto; agora publica por iniciativa, junto com o resto do julgamento do A10 virando por iniciativa (`3f8bdc1`, `7fcb354` — issue → projeto → iniciativa, tudo no mesmo dia). Isso na real *resolve* uma inconsistência que essa própria prep tinha (dizer "projeto" pro A10 quando o resto do doc já trata A10=iniciativa). |
| Onde o A14 publica hoje? | — | Via status update nativo do **projeto** (`publish_project_update`/`projectUpdateCreate`) — confirmado agora de vez: `commentCreate` não existe em nenhum lugar do repo, então a formulação antiga ("A14 em comentário na issue") está definitivamente errada, não só desatualizada. |
| O Slack já recebe algo hoje? | — | Só indiretamente: projetos/iniciativas do Linear podem ser plugados a canais do Slack, e quando o cron do A10/A14 falha ele publica um alerta que aparece automaticamente lá (confirmado ao vivo em 2026-09-03). Não existe hoje um push de insight (digest, status) dedicado ao Slack — só esse alerta de erro. **Exemplo real**: em 2026-09-03, um teste do cron gerou um alerta de verdade (`health="offTrack"`) que apareceu automaticamente no Slack do time — inclusive sem aviso de que era teste. |
| O outcome novo (loop A14→A10) aparece em algum lugar visível? | — | **Não, em nenhum.** `outcome_by_initiative`/`OutcomeSummary` são computados (`a10/rules.py::outcome_summary`) e trafegam até o payload da API/frontend (`frontend/src/api/types.ts`), mas **nenhum componente do frontend renderiza isso** e o texto publicado no Linear (`a10/formatting.py::format_portfolio_digest`) **não menciona outcome nenhum** — só as 4 sugestões de risco de sempre. E `chronic_no_effect`, que é exatamente o tipo de sinal que soaria como alerta ("entregando no prazo mas sem efeito, cronicamente"), **não alimenta nenhuma das 4 sugestões do A10** — fica calculado e parado. |
