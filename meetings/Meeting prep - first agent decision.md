...
Two reasons, both comparative rather than absolute:
...
| **A5** Watcher | Zero adoption risk; best-specified; one watcher covers every service behind the proxy | **No date** — blocked on GC-5, then an unscheduled Orca migration |
...
- **If Orca migrates soon → A5** gets a date and becomes competitive again on exactly
  the grounds it was first suggested.
