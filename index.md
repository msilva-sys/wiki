...
- [[Desenho do agente LangGraph para A10+A14]] — discovery em chat 2026-08-26 (repo `livemode-fluxo-agentico`, branch `langgraph`): padrão "agent" (`create_agent`) não "workflow", A10/A14 independentes, toolset determinístico-vs-julgamento, progresso do A14 por milestone (evita a subtask blindness já documentada), Langfuse Cloud só observabilidade, modelo OpenAI. Continuação mesma sessão: Pydantic pros contratos (`linear_client.py`/`contracts.py`), `poethepoet` (`pyproject.toml`) só como task runner (não troca `pip`+`venv`), cada agente roda isolado via `python -m a10.agent`/`a14.agent`. Mapeado em Linear PRO-392.
